...
Timeline._Band.prototype.setBandShiftAndWidth = function(shift, width) {
    ...
    if (this._timeline.isHorizontal()) {
        if(this.getIndex() == 1){
            var upperDivStyle = this.getTimeline().getBand(this.getIndex() - 1)._div.style;
            this._div.style.top = parseInt(upperDivStyle.height) + parseInt(upperDivStyle.top) + "px" ;
            this._div.style.minHeight = window.innerHeight  - parseInt(upperDivStyle.height) + parseInt(upperDivStyle.top) + "px";
        } else {
            this._div.style.top = shift + "px";
        }
        
        var trackIncrement = this._eventTrackIncrement;
        var trackCount = this._eventTracksNeeded;
        width = trackCount * trackIncrement +40; //40px for date markers
...
Timeline._Band.prototype.zoom = function(zoomIn, x, y) {
  if (!this._zoomSteps) {
    // zoom disabled
    return;
  }
  ...
  if (!this._lastScrollTime || ((now - this._lastScrollTime) > 50)) {
    ...
    if (mouseWheel === 'zoom') {
      ...
      if (delta != 0) {
        ...
        this._timeline.zoom(zoomIn, evt.clientX, evt.clientY, innerFrame);
      }
    }
...
Timeline._Band.prototype._scrollVertical = function(shift) {
    minTop = window.innerHeight - parseFloat(this._div.style.height);
    maxTop = parseFloat(this._timeline.getBand(0)._div.style.height);
    newTop = parseFloat(this._div.style.top) + shift;
    if(newTop < minTop){
        newTop = minTop;
