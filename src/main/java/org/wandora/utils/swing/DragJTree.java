 ...
 * need to implement allowDrop and doDrop. allowDrop checks if drop is allowed
 * in the location where user holds the mouse. doDrop is called when the user
 * finishes the drag and drop event. It must modify the model of the tree and
 * notify the tree of changes in the model.
 *
 * @author olli
 */
public abstract class DragJTree extends JTree implements Autoscroll {
        ...
        DragGestureListener dragGestureListener = new DragGestureListener() {
            public void dragGestureRecognized(DragGestureEvent dge) {
                ...
                if (path != null && !isRootPath(path)) {
                    ...
                    Component comp = getCellRenderer().getTreeCellRendererComponent(
                            DragJTree.this,
                    ...
                    ghostImage = new BufferedImage(
                            (int) pathRect.getWidth(), 
                            (int) pathRect.getHeight(),
     ...
     * 
     * @param destinationParent   The parent of the destination where user is about
     *                            to drop something.
     * @param destinationPosition The position after which user is about to drop
     *                            something. May be null in which case the item is
     *                            to be dropped as the first child of
     *                            destinationParent.
     * @param source              The item being dragged.
     * @return One of action constants in java.awt.dnd.DnDConstants representing the
    ...
    /**
     * Performs the drop event. Must modify the tree model to actually make the
     * changes in the tree and also notify the model of these changes so that the UI
     * can be updated.
     *
     * @param destinationParent   The parent of the destination where user is about
     *                            to drop something.
     ...
     *                            destinationParent.
     * @param source              The item being dragged.
     * @param action              The drop action. One of the action constants in
    ...
    /**
     * Override this method if you wish to process drag and drop events originating
     * from outside this component. Default implementation calls dtde.rejectDrop().
     */
    ...
    /**
     * Override this method if you wish to process drag and drop events originating
     * from outside this component. Default implementation calls dtde.rejectDrag().
     */
    ...
    /**
     * Override this method if you wish to process drag and drop events originating
     * from outside this component. Default implementation does nothing.
     */
    ...
    private class MyDropTargetListener implements DropTargetListener {
        ...
        public MyDropTargetListener() {
            ...
            hoverTimer = new Timer(3000, new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                    if (lastPath == null || isRootPath(lastPath))
                        return;
        ...
        public void drop(DropTargetDropEvent dtde) {
            ...
            if (accepted == action ||
                    (accepted == DnDConstants.ACTION_COPY_OR_MOVE
                            && (action == DnDConstants.ACTION_COPY || action == DnDConstants.ACTION_MOVE))) {
        ...
        public void dragOver(DropTargetDragEvent dtde) {
            ...
            if ((pt.y >= pathRect.y + pathRect.height / 4 || (row == 0 && DragJTree.this.isRootVisible()))
                    && pt.y < pathRect.y + pathRect.height * 3 / 4) {
            ...
            if (accepted == DnDConstants.ACTION_NONE) {
                ...
                if (next != null && next.getParentPath().getLastPathComponent() == path.getLastPathComponent()) {
                    ...
                    if (next == null || next.getParentPath().getLastPathComponent() != path.getParentPath()
    ...
    public static void main(String[] args) {
        ...
        w.add(new DragJTree() {
            public int allowDrop(TreePath destinationParent, TreePath destinationPosition, TreePath source) {
                ...
                if (isDescendant(source, destinationParent))
                    return DnDConstants.ACTION_NONE;
