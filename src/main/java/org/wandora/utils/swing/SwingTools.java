...
public class SwingTools {
    ...
    /**
     * Runs the given Runnable immediately if called from event dispatch thread or
     * at some later time inside event dispatch thread otherwise.
     */
    ...
    /**
     * Runs the given Runnable immediately if called from event dispatch thread or
     * at some later time inside event dispatch thread otherwise and blocks until it
     * is ready. Doesn't throw exceptions. Instead returns false in case of an
     * exception.
     */
    ...
    /**
     * Runs the given RunnableReturn immediately if called from event dispatch
     * thread or at some later time inside event dispatch thread otherwise. Returns
    ...
    public static <R> SwingReturn<R> swingOperation(final RunnableReturn<R> run) {
        if (!EventQueue.isDispatchThread()) {
            ...
            return new SwingReturn<R>(new Semaphore(1), new ValueContainer<R>(run.run()));
    }


    /**
     * Runs the given RunnableReturn immediately if called from event dispatch
     * thread or at some later time inside event dispatch thread otherwise. Blocks
     * until ready and returns value from RunnableReturn param.
     */
