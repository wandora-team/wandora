...
public class SQLProxyServer extends Thread {
    ...
    public SQLProxyServer(String dbDriver, String dbConnectionString, String dbUser, String dbPassword, Integer port,
            String proxyUser, String proxyPassword) {
        this(port.intValue());
        setCredentials(proxyUser + ":" + proxyPassword);
        addConnection("default", dbDriver, dbConnectionString, dbUser, dbPassword);
    ...
    public void addConnection(String key, String dbDriver, String dbConnectionString, String dbUser,
            String dbPassword) {
        if (dbDriver == null)
            dbDriver = guessDBDriver(dbConnectionString);
    ...
    public static String guessDBDriver(String dbConnectionString) {
        if (dbConnectionString.startsWith("jdbc:mysql:"))
            return "com.mysql.jdbc.Driver";
        else if (dbConnectionString.startsWith("jdbc:microsoft:sqlserver:"))
            return "com.microsoft.jdbc.sqlserver.SQLServerDriver";
        else if (dbConnectionString.startsWith("jdbc:hsqldb:"))
    ...
    public static void main(String[] args) {
        ...
        if (args.length > 4)
            server = new SQLProxyServer(args[0], args[1], args[2], args[3], Integer.parseInt(args[4]));
        else if (args.length > 3)
            server = new SQLProxyServer(args[0], args[1], args[2], args[3]);
        else if (args.length > 2)
            server = new SQLProxyServer(args[0], args[1], args[2]);
        else {
            printUsage();
            return;
        }
...
//        server.setEcho(true);
    ...
    public static final String RES_AUTHREQUIRED = "4"; // command requires authentication or invalid credentials when
                                                       // authenticating
    public static final String RES_CONREQUIRED = "5"; // command requires an open database connection or invaling
                                                      // connection key when opening connection
                            ...
                            if (query.startsWith("open")) {
                                ...
                                while (rs.next()) {
                                    for (int i = 0; i < columnCount; i++) {
                                        if (i > 0)
                                            outWriter.write(",");
                                        if (isTypeNumeric(types[i]))
                                            outWriter.write("" + rs.getString(i + 1));
                                        else
                                            writeString(rs.getString(i + 1));
                                    }
                                    outWriter.write(lf);
                                }
                                outWriter.write(lf);
