...
public class SQLProxyClient {
     ...
     */
    public static SQLProxyClient createProxy(String connectionString, String user, String password)
            throws IOException, SQLProxyException {
        if (!connectionString.startsWith("sqlproxy:"))
            return null;

        connectionString = connectionString.substring("sqlproxy:".length());
        if (connectionString.startsWith("//"))
            connectionString = connectionString.substring(2);
    ...
    private Collection<Map<String, Object>> readResultSet() throws IOException, SQLProxyException {
        ...
        if (!line.equals(SQLProxyServer.RES_RESULTSET))
            throw new SQLProxyException("Unexpected return type " + line);
        line = readLine();
    ...
    public int readUpdateCount() throws IOException, SQLProxyException {
        String line = readLine();
        if (!line.equals(SQLProxyServer.RES_COUNT))
            throw new SQLProxyException("Unexpected return type " + line);
        line = readLine();
        try {
    ...
    public synchronized boolean login(String user, String pass) throws IOException {
        ...
        outWriter.flush();
        if (compress)
            inReader = new PushbackReader(new InputStreamReader(new GZIPInputStream(socket.getInputStream()), charset));
        String line = readLine();
        boolean ret = false;
        if (line.equals(SQLProxyServer.RES_OK))
            ret = true;
        if (compress)
