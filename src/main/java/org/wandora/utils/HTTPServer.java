...
public abstract class HTTPServer {
    ...
    protected void handleRequest(Socket s) throws IOException {
        ...
        if (loginUser != null)
            requiredCredentials = loginUser + ":" + loginPass;
    ...
    public void writeHeaderForFile(OutputStream out, String extension) throws IOException {
        String type = null;
        if (extension != null)
            type = mimeTypes.get(extension.toLowerCase());
        if (type == null)
            type = "text/plain";
    ...
    public void writeHeaderForFile(OutputStream out, File f) throws IOException {
        ...
        if (ind == -1)
            writeHeaderForFile(out, (String) null);
        else
            writeHeaderForFile(out, name.substring(ind + 1));
    }
    ...
    public static void writeInternalServerError(OutputStream out, String message, Throwable t) throws IOException {
        ...
        t.printStackTrace(p);
        p.flush();
        writeSimpleHTML(out, "500 Internal Server Error", "500 Internal Server Error<br />"
    ...
    public static String[] parseGetParams(String get) {
        try {
            int ind = get.indexOf("?");
            if (ind == -1)
                return new String[] { URLDecoder.decode(get, "UTF-8") };
