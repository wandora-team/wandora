...
public class W2TTopicMap implements TopicMap {
    ...
    */
    ...
    public Set<Locator> wrapLocators(Collection<org.wandora.topicmap.Locator> locators) {
        Set<Locator> ret = new LinkedHashSet<>();
    ...
    public Locator getLocator() {
        if (locator != null)
            ...
            return new W2TLocator(DEFAULT_TM_LOCATOR);
    }
    ...
    @Override
    public Topic createTopicByItemIdentifier(Locator lctr)
    ...
    public Association createAssociation(Topic type, Topic... scope) throws ModelConstraintException {
        if (scope != null && scope.length > 0)
            throw new UnsupportedOperationException("Scope not supported in associations");
        try {
    ...
    public <I extends Index> I getIndex(Class<I> type) {
        if (type.isAssignableFrom(TypeInstanceIndex.class))
            return (I) (new W2TTypeInstanceIndex());
        else if (type.isAssignableFrom(ScopedIndex.class))
            return (I) (new W2TScopedIndex());
        else
            throw new UnsupportedOperationException("Index not supported.");
    }
    ...
    private class W2TTypeInstanceIndex implements TypeInstanceIndex {
        ...
        public Collection<Role> getRoles(Topic topic) {
            try {
                ...
                Collection<org.wandora.topicmap.Association> as = ((W2TTopic) topic).getWrapped()
                ...
                for (org.wandora.topicmap.Association a : as) {
                    ...
                    ret.add(new W2TRole(new W2TAssociation(W2TTopicMap.this, a), (W2TTopic) topic,
                            new W2TTopic(W2TTopicMap.this, p)));
                }
                return ret;
            }
            catch (TopicMapException tme) {
        ...
        public Collection<Occurrence> getOccurrences(Topic topic) {
            if (topic == null)
                return new ArrayList<Occurrence>(); // occurrences without type are not supported in Wandora so this is always empty
            try {
        ...
        public Collection<Name> getNames(Topic topic) {
            if (topic != null)
                return new ArrayList<Name>(); // all names in wandora are typeless
            else {
                try {
                    ...
                    while (iter.hasNext()) {
                        ...
                        if (t.getBaseName() != null)
                            ret.add(new W2TName(W2TTopicMap.this, new W2TTopic(W2TTopicMap.this, t)));
                    }
                    return ret;
                }
                catch (TopicMapException tme) {
    ...
    private class W2TScopedIndex implements ScopedIndex {
        ...
        public Collection<Occurrence> getOccurrences(Topic topic) {
            if (topic == null)
                throw new UnsupportedOperationException("Not supported yet."); // should return all occurrences
            org.wandora.topicmap.Topic _topic = ((W2TTopic) topic).getWrapped();
            try {
                ArrayList<Occurrence> ret = new ArrayList<Occurrence>();
        ...
        public Collection<Occurrence> getOccurrences(Topic[] topics, boolean matchAll) {
            if (topics.length == 0) {
                if (matchAll)
                    return getOccurrences(null); // for now this throws an exception but this should do the right thing when implemented
                else
                    return new ArrayList<Occurrence>();
            }
            else if (topics.length == 1)
                return getOccurrences(topics[0]);
            else { // .length>1
                if (matchAll)
                    return new ArrayList<Occurrence>(); // no occurrence in wandora has two versions so this is always empty
        ...
        public Collection<Name> getNames(Topic topic) {
            if (topic == null)
                throw new UnsupportedOperationException("Not supported yet."); // should return all names
            else
        ...
        public Collection<Variant> getVariants(Topic topic) {
            if (topic == null)
                return getVariants(new Topic[0], false);
            else
                return getVariants(new Topic[] { topic }, false);
        }
            ...
            if (topics.length == 0) {
                ...
                if (topics.length < 2 || !matchAll) {
                    try {
                        ...
                        if (topics.length == 1)
                            ts = ((W2TTopic) topics[0]).getWrapped().getTopicsWithVariantScope();
                        else {
                            ts = new TopicHashSet();
                            for (Topic topic : topics) {
