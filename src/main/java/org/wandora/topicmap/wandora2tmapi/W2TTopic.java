...
public class W2TTopic implements Topic {
    ...
    */
    ...
    public Set<Locator> getSubjectLocators() {
        try {
            ...
            Set<Locator> ret = new LinkedHashSet<Locator>();
            if (l != null)
    ...
    public void addSubjectLocator(Locator lctr) throws IdentityConstraintException, ModelConstraintException {
        try {
            if (t.getSubjectLocator() == null)
                t.setSubjectLocator(tm.tm.createLocator(lctr.toExternalForm()));
            else
                throw new UnsupportedOperationException("Multiple subject locators in a topic not supported");
        }
        catch (TopicMapException tme) {
            throw new TMAPIRuntimeException(tme);
        }
    ...
    public void removeSubjectLocator(Locator lctr) {
        try {
            if (t.getSubjectLocator() != null && lctr != null) {
                if (t.getSubjectLocator().toExternalForm().equals(lctr.toExternalForm()))
    ...
    */
    ...
    public Set<Name> getNames() {
        Set<Name> ret = new LinkedHashSet<Name>();
        Name name = _getName();
        if (name != null)
            ret.add(name);
        return ret;
    ...
    public Set<Name> getNames(Topic topic) {
        ...
        if (topic.getSubjectIdentifiers().contains(tm.createLocator(W2TTopicMap.TOPIC_NAME_SI)))
            return getNames();
        else
    ...
    */
    ...
    public Name createName(Topic type, String value, Topic... scope) throws ModelConstraintException {
        if (_getName() != null)
            throw new UnsupportedOperationException("Only one name per topic is supported");
        else {
            if (scope.length > 0)
                throw new UnsupportedOperationException("Scoped names are not supported");
    ...
    public Name createName(String value, Collection<Topic> scope) throws ModelConstraintException {
        return createName(tm.getTopicBySubjectIdentifier(W2TTopicMap.TOPIC_NAME_SI), value,
                scope.toArray(new Topic[0]));
    }
    ...
    public Set<Occurrence> getOccurrences() {
        try {
            Set<Occurrence> ret = new LinkedHashSet<>();
    ...
    */
    ...
    public Occurrence createOccurrence(Topic type, String value, Locator dataType, Topic... scope)
            throws ModelConstraintException {
        if (!dataType.toExternalForm().equals(W2TTopicMap.TYPE_STRING_SI))
            throw new UnsupportedOperationException("Only string occurrences are supported");
        if (scope.length != 1)
            throw new UnsupportedOperationException("The scope must have exactly one topic indicating the language");
        ...
        try {
            ...
            if (data != null)
                throw new UnsupportedOperationException("Only one occurrence for each type/language pair allowed");
    ...
    protected Set<Role> _getRolesPlayed(Topic type, Topic associationType) {
        try {
            ...
            if (associationType == null)
                as = t.getAssociations();
            else
                as = t.getAssociations(((W2TTopic) associationType).t);
            ...
            for (org.wandora.topicmap.Association a : as) {
                ...
                for (org.wandora.topicmap.Topic role : a.getRoles()) {
                    if (type != null && !((W2TTopic) type).t.mergesWithTopic(role))
                        continue;
    ...
    public void remove() throws TopicInUseException {
        try {
            if (!t.isDeleteAllowed())
                throw new TopicInUseException(this, "Topic in use");
            else
                t.remove();
        }
        catch (TopicMapException tme) {
