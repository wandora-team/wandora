...
public class JTMParser {
            ...
            if (key != null) {
                ...
                if ("version".equals(key)) {
                    ...
                    if (areEqual("topicmap", value)) {
                        ...
                        log("Warning: Wandora's JTM parser does not support item_type '" + value.toString()
                                + "'. Skipping.");
                    }
                }
    ...
    public void parseTopic(JSONObject topicJSON) throws Exception {
        ...
        t.addSubjectIdentifier(
                new Locator(STATIC_SI_BODY + System.currentTimeMillis() + "-" + Math.floor(Math.random() * 9999999)));
                            ...
                            if (sl != null) {
                                ...
                                if (sls != null && sls.length() > 0) {
                                    if (i < 1) {
                                        ...
                                        log("Warning: Wandora supports only one subject locator in topic. Skipping subject locator '"
                                                + sl + "'.");
                                    }
                                }
                            }
                        ...
                        for (int i = 0; i < length; i++) {
                            ...
                            if (ii != null) {
                                ...
                                if (iis != null && iis.length() > 0) {
                                    log("Warning: Wandora doesn't support item identifiers. Making subject identifier out of item identifier '"
                                            + ii + "'.");
            ...
            if (key != null) {
                ...
                if ("value".equals(key)) {
                    ...
                    if (value instanceof JSONArray) {
                        ...
                        for (int i = 0; i < length; i++) {
                            if (scope != null) {
                                log("Warning: Wandora supports only one scope topic in occurrences. Skipping scope '"
                                        + scopeArray.getString(i));
                            }
                            else {
