...
public class ModifyAssociationOperation extends UndoOperation {
    ...
    public static ModifyAssociationOperation setType(Association a, Topic type)
    ...
    public static ModifyAssociationOperation addPlayers(Association a, Map<Topic, Topic> members)
            throws TopicMapException, UndoException {
        ModifyAssociationOperation ret = new ModifyAssociationOperation();
        ret.removeOperation = new RemoveAssociationOperation(a);
        ret.addOperation = new CreateAssociationOperation(a, false);
        ...
        Map<Locator, Locator> players = ret.addOperation.getPlayers();
        for (Map.Entry<Topic, Topic> e : members.entrySet()) {
            if (ret.nop) {
                Topic oldPlayer = a.getPlayer(e.getKey());
                if (oldPlayer == null || !e.getValue().mergesWithTopic(oldPlayer))
                    ret.nop = false;
            }
    ...
    public static ModifyAssociationOperation addPlayer(Association a, Topic role, Topic player)
            throws TopicMapException, UndoException {
        ModifyAssociationOperation ret = new ModifyAssociationOperation();
        ...
        Map<Locator, Locator> players = ret.addOperation.getPlayers();
        if (player == null)
            players.remove(role.getFirstSubjectIdentifier());
        else
            players.put(role.getFirstSubjectIdentifier(), player.getFirstSubjectIdentifier());
        ret.addOperation.setPlayers(players);
        return ret;
    }


    public static ModifyAssociationOperation removePlayer(Association a, Topic role)
            throws TopicMapException, UndoException {
