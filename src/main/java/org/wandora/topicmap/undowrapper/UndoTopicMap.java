...
public class UndoTopicMap extends TopicMap {
    ...
    List<Topic> wrapTopics(Collection<Topic> topics) {
    ...
    public void setParentTopicMap(TopicMap parent) {
        if (wrapped != null)
            wrapped.setParentTopicMap(parent);
    }
    ...
    public Iterator<Topic> getTopics() throws TopicMapException {
        ...
        return new TopicIterator() {
            @Override
            public void dispose() {
                if (iter instanceof TopicIterator)
                    ((TopicIterator) iter).dispose();
                else
                    while (iter.hasNext())
                        iter.next();
            }
    ...
    private Map<TopicMapListener, TopicMapListenerWrapper> wrappedListeners = new LinkedHashMap<TopicMapListener, TopicMapListenerWrapper>();
    ...
    private class TopicMapListenerWrapper implements TopicMapListener {
        ...
        public void topicSubjectLocatorChanged(Topic t, Locator newLocator, Locator oldLocator)
                throws TopicMapException {
        ...
        public void associationPlayerChanged(Association a, Topic role, Topic newPlayer, Topic oldPlayer)
                throws TopicMapException {
            wrapped.associationPlayerChanged(wrapAssociation(a), wrapTopic(role), wrapTopic(newPlayer),
                    wrapTopic(oldPlayer));
        }
