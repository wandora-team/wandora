...
public class UndoBuffer {
    private static final Log4j2Logger logger = Log4j2Logger.getLogger(UndoBuffer.class);
    ...
    public void addOperation(UndoOperation op) {
        ...
        if (!undobuffer.isEmpty()) {
            ...
            if (SKIP_EMPTY_OPERATIONS && previous.isMarker() && op.isMarker())
                return;
            ...
            if (SKIP_EMPTY_OPERATIONS && op.isMarker())
                return;
        }
        ...
        if (undobuffer.size() > MAX_BUFFER_SIZE) {
            ...
            logger.info("Removing oldest operations in the undo buffer");
    ...
    public int getUndoOperationNumber() {
        if (undobuffer.isEmpty())
            ...
            return undobuffer.peekLast().getOperationNumber();
    }
    ...
    public int getRedoOperationNumber() {
        if (redobuffer.isEmpty())
            ...
            return redobuffer.peekFirst().getOperationNumber();
    }
