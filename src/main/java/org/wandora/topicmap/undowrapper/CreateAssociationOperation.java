...
public class CreateAssociationOperation extends UndoOperation {
    ...
    private Map<Locator, Locator> players;
    private boolean associationAlreadyExists = false;

    public CreateAssociationOperation(TopicMap tm, Locator type, Map<Locator, Locator> players)
            throws UndoException, TopicMapException {
        this.tm = tm;
        this.type = type;
    ...
    public CreateAssociationOperation(Topic type, Map<Topic, Topic> players)
            throws UndoException, TopicMapException {
        this.tm = type.getTopicMap();
        this.type = type.getOneSubjectIdentifier();
        this.players = new LinkedHashMap<Locator, Locator>();
    ...
    public CreateAssociationOperation(TopicMap tm, Locator type, Locator role, Locator player)
            throws UndoException, TopicMapException {
        this.tm = tm;
        this.type = type;
        this.players = new LinkedHashMap<Locator, Locator>();
    ...
    public CreateAssociationOperation(Topic type, Topic role, Topic player) throws UndoException, TopicMapException {
        ...
        if (this.type == null || rsi == null || psi == null)
            throw new UndoException();
        players.put(rsi, psi);
    ...
    private CreateAssociationOperation(TopicMap tm, Locator type, Map<Locator, Locator> players, boolean exists) {
        this.tm = tm;
        this.type = type;
        this.players = players;
        this.associationAlreadyExists = exists;
    ...
    Map<Locator, Locator> getPlayers() {
        return players;
    }


    void setPlayers(Map<Locator, Locator> players) {
        this.players = players;
        try {
            Association oa = RemoveAssociationOperation.findAssociation(tm, type, players);
            associationAlreadyExists = (oa != null);
    ...
    public void redo() throws UndoException {
        try {
            if (!associationAlreadyExists) {
                ...
                if (t == null)
                    throw new UndoException();
                Map<Topic, Topic> ps = new LinkedHashMap<>();
    ...
    public CreateAssociationOperation clone() {
        Map<Locator, Locator> pclone = new LinkedHashMap<>();
