  ...
  public boolean parseCalendar(net.fortuna.ical4j.model.Calendar calendar, TopicMap topicMap) {
    try {
      ...
      if (prodId != null) {
        ...
        //Parse ToDos
        List<CalendarComponent>  todos = calendar.getComponents("VTODO");
        if (todos != null) {
          for (CalendarComponent component : todos) {
    ...
    try {
      ...
      if (action != null && trigger != null) {
        ...
        if (trigger.getParameter("VALUE") != null && trigger.getParameter("VALUE").getValue().equals("DATE-TIME")) {
          ...
          TemporalAmount d = trigger.getDuration();
          List<TemporalUnit> tus = d.getUnits();
          for(TemporalUnit u : tus) {
        	  if(d.get(u) != 0) {
        		  if(!trigString.isEmpty()) {
        			  trigString += " ";
        		  }
        		  trigString += d.get(u) + " " + u.toString();
        	  }
          }
        }
