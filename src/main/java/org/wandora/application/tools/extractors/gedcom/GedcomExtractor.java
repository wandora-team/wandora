...
/**
 * Converts GEDCOM (GEnealogical Data COMmunication) formatted files to topic
 * maps.
 *
 * Extractor is based on Mike Dean's work on GEDCOM to DAML converter. DAML
 * converter is described at http://www.daml.org/2001/01/gedcom/
 *
 * @author akivela
 */
...
public class GedcomExtractor extends AbstractExtractor implements WandoraTool {
	...
	public String getGUIText(int textType) {
		...
		case SELECT_DIALOG_TITLE:
			return "Select GEDCOM data file(s) or directories containing GEDCOM data files!";
		case POINT_START_URL_TEXT:
			return "Where would you like to start the crawl?";
		case INFO_WAIT_WHILE_WORKING:
			return "Wait while seeking GEDCOM data files!";

		case FILE_PATTERN:
			return ".*\\.ged";

		case DONE_FAILED:
			return "Ready. No extractions! %1 GEDCOM data file(s) crawled!";
		case DONE_ONE:
			return "Ready. Successful extraction! %1 GEDCOM data file(s) crawled!";
		case DONE_MANY:
			return "Ready. Total %0 successful extractions! %1 GEDCOM data file(s) crawled!";

		case LOG_TITLE:
			return "GEDCOM extraction Log";
		}
		return "";
	}
	...
	public boolean _extractTopicsFrom(BufferedReader breader, TopicMap topicMap) throws Exception {
		...
		try {
			...
			Iterator<Gedcom.Node> iterator = gedcom.keys.values().iterator();
		...
		if (nobje > 0)
			log("Skipped " + nobje + " object records");
		if (nnote > 0)
			log("Skipped " + nnote + " note records");
		if (nrepo > 0)
			log("Skipped " + nrepo + " repository records");
		if (nsubm > 0)
			log("Skipped " + nsubm + " submitter records");
		return true;
	}
	...
	private void processIndividual(Gedcom.Node node, TopicMap tm) throws TopicMapException {
		...
		while (childIterator.hasNext()) {
			...
			if ("SEX".equals(child.tag)) {
				...
				Association assoAssociation = addAssociation(tm, "associated-individual", individualTopic,
						individualTypeTopic, assoTopic, "associated-individual");
				Iterator<Gedcom.Node> childchildIterator = child.children.iterator();
			...
			// --- INDIVIDUAL EVENTS ---
				...
				Association eventAssociation = processEvent(child, tm, "adoptation", individualTopic,
						individualTypeTopic);
				Iterator<Gedcom.Node> childchildIterator = child.children.iterator();
				while (childchildIterator.hasNext()) {
					Gedcom.Node childchild = (Gedcom.Node) childchildIterator.next();
			...
			/*
			 * else if("FAMS".equals(child.tag)) { Topic familyTopic = getOrCreateTopic(tm,
			 * fixNodeKey(child.value)); Topic familyTypeTopic = getOrCreateFamilyType(tm);
			 * addAssociation(tm, "spouse", familyTopic, familyTypeTopic, individualTopic,
			 * "spouse"); }
			 */
	...
	private Association processEvent(
			Gedcom.Node node, 
			TopicMap tm, 
			String eventType, 
			Topic baseTopic,
			Topic baseTypeTopic) throws TopicMapException {
		...
		while (childIterator.hasNext()) {
			...
			if ("PLAC".equals(child.tag)) {
				...
				Topic noteTopic = getOrCreateTopic(tm,
						"note/" + System.currentTimeMillis() + ((int) Math.random() * 10000), noteBaseName);
	...
	private void processName(
		...
		Iterator<Gedcom.Node> nameIterator = node.children.iterator();
	...
	public void addOccurrence(TopicMap tm, Topic carrier, String occurrenceType, String occurrenceText)
			throws TopicMapException {
		if (tm == null)
			return;
		if (carrier == null)
			return;
		if (occurrenceType == null || occurrenceText == null)
			return;
	...
	public Association fillAssociation(TopicMap tm, Association association, Topic playerTopic, String role)
			throws TopicMapException {
		if (tm == null)
			return null;
		if (playerTopic == null || role == null)
			return null;
	...
	public Association fillAssociation(TopicMap tm, Association association, String player, String role)
			throws TopicMapException {
		if (tm == null)
			return null;
	...
	public Association addAssociation(TopicMap tm, String associationType, Topic player1Topic, Topic role1Topic)
			throws TopicMapException {
		if (tm == null)
			return null;
		if (associationType == null)
			return null;
		if (player1Topic == null || role1Topic == null)
			return null;
	...
	public Association addAssociation(TopicMap tm, String associationType, Topic player1Topic, Topic role1Topic,
			String player2, String role2) throws TopicMapException {
		if (tm == null)
			return null;
		if (associationType == null)
			return null;
	...
	public Association addAssociation(TopicMap tm, String associationType, Topic player1Topic, Topic role1Topic,
			Topic player2Topic, String role2) throws TopicMapException {
		if (tm == null)
			return null;
		if (associationType == null)
			return null;
		if (player1Topic == null || role1Topic == null)
			return null;
		if (player2Topic == null || role2 == null)
			return null;
		...
		try {
			...
			for (Iterator<Association> i = c.iterator(); i.hasNext();) {
				...
				for (Iterator<Topic> it = roles.iterator(); it.hasNext();) {
					...
					if (player != null && t2.mergesWithTopic(player))
						return true;
				}
			}
		} catch (Exception e) {
