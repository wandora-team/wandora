...
Timeline.OverviewEventPainter.prototype.paintInstantEvent = function(evt, metrics, theme, highlightIndex) {
    ...
    var endPixel = startPixel+1;    
    var tapeTrack = 0;
        for (; tapeTrack < this._tracks.length; tapeTrack++) {
            if (endPixel < this._tracks[tapeTrack]) {
                break;
            }
    }
    this._tracks[tapeTrack] = startPixel;

    var color = evt.getColor();
    color = color != null ? color : theme.event.duration.color;
    
    var tickElmtData = this._paintEventTick(evt, tapeTrack,startPixel, color, 100, metrics, theme);
...
Timeline.OverviewEventPainter.prototype._paintEventTick = function(
    evt, track, left, color, opacity, metrics, theme) {

    //left = isNaN(left) ? 0 : left;
    
    var height = theme.event.overviewTrack.tickHeight;
    var top = metrics.trackOffset +track * metrics.trackIncrement-height;
    var width = theme.event.overviewTrack.tickHeight;
    ...
    tickDiv.style.top = top.toString() + "px";
