 ...
 * @author  olli, akivela
 */
public class OccurrenceTable extends SimpleTable implements MouseListener, Clipboardable {
    public static final String OPTIONS_KEY = "gui.occurrenceTable";
    public static final String TYPE_OPTIONS_KEY = "gui.occurrenceTable.type";
    ...
    public static final String TYPE_USED_SCHEMA = "used+schema";
    
    public String tableType = TYPE_SCHEMA;
    ...
    /** Creates a new instance of OccurrenceTable */
    public OccurrenceTable(Topic topic,Wandora wandora) throws TopicMapException {
        this.wandora=wandora;
        this.topic=topic;
        
        try {
            Options opts = wandora.getOptions();
            if(opts != null) {
                tableType = opts.get(TYPE_OPTIONS_KEY);
                if(tableType == null || tableType.length() == 0) tableType = TYPE_SCHEMA;
        ...
        if(TYPE_USED.equalsIgnoreCase(tableType) || TYPE_USED_SCHEMA.equalsIgnoreCase(tableType)) {
        ...
        if(TYPE_SCHEMA.equalsIgnoreCase(tableType) || TYPE_USED_SCHEMA.equalsIgnoreCase(tableType)) {
    ...
    private void initPopupStructures() {
        try {
            popupStruct = WandoraMenuManager.getOccurrenceTableMenu(this);
        }
        catch(Exception tme){
            ...
            if(realRow >= 0 && realCol > 0 && realRow < types.length && realCol < langs.length+1) {
                ...
                for(int i=0; i<langs.length; i++) {
                    if(i != realCol-1) {
                        ...
                        String translatedOccurrenceText = GoogleTranslateBox.translate(occurrenceText, sourceLangTopic, targetLangTopic, markTranslation);
        ...
        try {
            ...
            if(realRow >= 0 && realCol > 0 && realRow < types.length && realCol < langs.length+1) {
                ...
                for(int i=0; i<langs.length; i++) {
                    if(i != realCol-1) {
                        Topic targetLangTopic = langs[i];
                        String translatedOccurrenceText = MicrosoftTranslateBox.translate(occurrenceText, sourceLangTopic, targetLangTopic, markTranslation);
                        if(translatedOccurrenceText != null) {
                            int override = WandoraOptionPane.YES_OPTION;
                            if(!overrideAll && topic.getData(type, targetLangTopic) != null) {
                                override = WandoraOptionPane.showConfirmDialog(wandora, "Override existing "+targetLangTopic.getDisplayName()+" occurrence?", "Override existing occurrence?", WandoraOptionPane.YES_TO_ALL_NO_CANCEL_OPTION);
